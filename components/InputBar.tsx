...
export const InputBar: React.FC<InputBarProps> = ({ onAdd, onMagicAdd, isGenerating }) => {
  ...
  return (
    <div className="w-full bg-white p-2 rounded-[2rem] shadow-[0_4px_20px_-2px_rgba(0,0,0,0.1)] border border-gray-100 transition-all duration-200 hover:shadow-[0_8px_25px_-4px_rgba(0,0,0,0.12)] flex items-center gap-2 relative z-20">
        {/* Input Field */}
        <input
            type="text"
            value={inputValue}
            ...
            className="flex-1 bg-transparent text-gray-900 placeholder-gray-400 text-base sm:text-lg font-medium border-none px-4 py-2 focus:ring-0 focus:outline-none min-w-0"
        ...
        <div className="flex items-center gap-2 pr-2 shrink-0">
            ...
            <div className="relative group/date h-10">
                ...
                <button
                    type="button"
                    className={`h-full flex items-center gap-2 px-4 rounded-full text-sm font-medium transition-all duration-200 select-none ${
                        dueDate 
            ...
            <button
                ...
                disabled={!inputValue.trim() || isGenerating}
                className={`flex items-center justify-center w-10 h-10 rounded-full transition-all duration-200 shadow-sm transform ${
                    inputValue.trim() && !isGenerating
